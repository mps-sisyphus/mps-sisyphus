...
public class MPSBuild implements ITask {
    private static final Logger logger = Logger.instance();

    private final String name;
    private final Path antFile;

    private MPSBuild(final String name, final Path antFile) {
        this.name = name;
        this.antFile = antFile;
    }

    public static MPSBuild load(final Node node) {
        ...
        return new MPSBuild(name.getNodeValue(), Path.of(antFile.getNodeValue()));
    }

    public boolean run(final Path workingDir, final Path sisyphusMpsPlugins) {
        logger.info("-".repeat(80));
        logger.info(String.format(" ANT build (MPS): '%s' ('%s')", name, antFile));
        logger.info("-".repeat(80));
        ...
        if (exitCode == 0) {
            ...
            logger.error(String.format("ANT: '%s' ('%s') FAILED", name, antFile));
            return false;
        }
    }
